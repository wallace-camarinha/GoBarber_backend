...
let CreateAppointmentService = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute({
    ...
    if ((0, _dateFns.isBefore)(appointmentDate, Date.now())) {
      throw new _AppError.default('You can not create an appointment on a past date', 400);
    }
    ...
    if ((0, _dateFns.getHours)(appointmentDate) < 8 || (0, _dateFns.getHours)(appointmentDate) > 17) {
      throw new _AppError.default('You can only create an appointment between 8am and 5pm', 400);
    }

    const findAppointmentInSameDate = await this.appointmentsRepository.findByDate(appointmentDate);

    if (findAppointmentInSameDate) {
      throw new _AppError.default('This hour is already booked');
    }
